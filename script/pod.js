...
let files = ["2b_sun.jpg", "2b_violin.jpg", "a2.png", "nier_rest.png", "yorha.jpg"];
let active = "#home";
let ac_app = "#piracy";
let link_data;
let anime_data;
...
function quote() {
  var quotes = ['黒蜥蜴星から来たの','お帰りなさい、<BR/>ご主人様', 'トゥットゥルー♪', 'エル・プサイ・コングルゥ', '宇宙には始まりはあるが、<BR/>終わりはない', '何でもは知らないわよ、<BR/>知ってることだけ', '僕はキメ顔でそう言った', '私気になります', 'やれやれだぜ', 'お可愛いこと', '失礼、噛みました', 'にゃんぱす～'];
...
function get_weather() {
  let url = "https://api.openweathermap.org/data/2.5/weather?id=" + city_code + "&units=metric" + "&lang=ja" + "&APPID=" + api_key;
  $.get(url, function(data) {
    ...
    weather_data = ["Forecast", temp_max, temp_min, condition, cur_temp, time];
...
function get_nihon_date() {
    var date = new Date();
    var month = date.toLocaleDateString('ja-JP', {
        month: 'long'
    });
    var day = date.toLocaleDateString('en', {
        day: '2-digit'
    });
    var weekday = date.toLocaleDateString('ja-JP', {
        weekday: 'long'
    });
...
function load_links(cat) {
    ...
    $(cat).parent().addClass("title-box-container-selected");
    $(cat).addClass("title-box-selected");
    $(cat.concat("-span")).addClass("title-span-selected");
...
function load_anime(anime_data) {
    ...
    if (airing.length == 0) {
        ...
        str = airing[0] + " is airing today."
    } else {
        str;
        for (let i = 0; i < airing.length - 2; i++) {
            str += airing[i] + ", "
        }
        str += airing[airing.length - 1] + " is airing today."
    }
...
function remove_selection(title) {
    ...
    $(span).removeClass("title-span-selected");
    $(title).parent().removeClass("title-box-container-selected");
...
function darkify() {
    let img_str = active.concat("-img").replace("#", "");
    let img_name = "img/".concat(active).concat("_dark.png").replace("#", "");
    document.getElementById(img_str).src = img_name
}
